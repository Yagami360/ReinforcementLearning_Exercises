...
class DDQNMLPBrain( Brain ):
    ...
    def loss( self ):
        ...
        self._loss_fn = F.smooth_l1_loss(
            input = self._q_function,                        # 行動価値関数 Q(s,a;θ) / shape = [n_batch,1]
            target = self._expected_q_function.unsqueeze(1)  # 推定行動価値関数 Q(s,a;θ) / unsqueeze(1) で [n_batch] → [n_batch,1] に reshape
        )
